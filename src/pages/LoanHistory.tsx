  ...
  if (loading && !initialized) {
    return (
      <div className="space-y-5">
        <div className="space-y-1"><Skeleton className="h-7 w-36" /><Skeleton className="h-4 w-52" /></div>
        <Card>
          <CardHeader className="pb-2"><Skeleton className="h-4 w-56" /></CardHeader>
          <CardContent><Skeleton className="h-52 w-full" /></CardContent>
        </Card>
        <div className="flex gap-2"><Skeleton className="h-8 w-48" /><Skeleton className="h-8 w-32" /><Skeleton className="h-8 w-28" /></div>
        <div className="rounded-lg border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                {['Customer', 'Loan #', 'Loan Amount', 'Period', 'Progress', 'Paid', 'Pending', 'Status', ''].map(h => (
                  <th key={h} className="px-4 py-2.5 text-left"><Skeleton className="h-3 w-16" /></th>
                ))}
              </tr>
            </thead>
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        {refreshing && <p className="text-xs text-muted-foreground">Refreshing loan history...</p>}
      </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                ...
                <YAxis tickLine={false} axisLine={false} width={52}
                  tickFormatter={v => v >= 1000 ? `₹${(v / 1000).toFixed(0)}k` : `₹${v}`} tick={{ fontSize: 10 }} />
                <Tooltip formatter={(value) => {
                  let amount = 0
                  if (typeof value === 'number') amount = value
                  else if (typeof value === 'string' && !Number.isNaN(Number(value))) amount = Number(value)
      ...
      <div className="rounded-lg border overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-muted/50">
            <tr>
              {['Customer', 'Loan #', 'Loan Amount', 'Period', 'Progress', 'Paid', 'Pending', 'Status', ''].map(h => (
                <th key={h} className={`text-left px-4 py-2.5 font-medium text-muted-foreground text-xs ${h === 'Pending' ? 'text-orange-500' : ''} ${h === 'Period' ? 'hidden sm:table-cell' : ''}`}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y">
            {filtered.length === 0 ? (
              <tr><td colSpan={9} className="text-center py-10 text-muted-foreground text-sm">No loans found</td></tr>
            ) : (
              filtered.map(loan => {
                ...
                return (
                  <tr key={loan.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-muted-foreground">{getLoanNumber(loan)}</td>
                    <td className="px-4 py-3 font-semibold">₹{loan.loanAmount.toLocaleString('en-IN')}</td>
                    <td className="px-4 py-3 hidden sm:table-cell text-xs text-muted-foreground">
                      <p>{loan.startDate}</p><p>{loan.endDate}</p>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${loan.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}`}>
                        {loan.status}
                      </span>
                    </td>
                    <td className="px-4 py-3">
