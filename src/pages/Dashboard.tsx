...
export default function Dashboard() {
  const { customers, loans, entries, loading, initialized, getLoanNumber } = useFinance()
  ...
  const weekData = week.map(date => ({
    date: date.slice(5),
    amount: entries.filter(e => e.date === date && e.type === 'payment').reduce((s, e) => s + e.amount, 0),
  }))
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2 flex-wrap">
        ...
        {refreshing && <span className="text-xs text-muted-foreground self-center">Refreshing dashboard data…</span>}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <KpiCard title="Total Disbursed" value={`₹${totalDisbursed.toLocaleString('en-IN')}`}
          description={`${loans.length} loans total`} icon={IndianRupee} trend="up"
          trendValue={`${activeLoans.length} active`} />
        <KpiCard title="Total Collected" value={`₹${totalCollected.toLocaleString('en-IN')}`}
          description={totalDisbursed > 0 ? `${Math.round((totalCollected / totalDisbursed) * 100)}% recovered` : '—'}
          icon={Wallet} trend="up" trendValue={`Today: ₹${todayTotal.toLocaleString('en-IN')}`} />
        <KpiCard title="Pending Amount" value={`₹${totalPending.toLocaleString('en-IN')}`}
          description={`${activeLoans.length} active loans`} icon={Wallet}
          trend={overdueLoans.length > 0 ? 'down' : 'up'}
          trendValue={overdueLoans.length > 0 ? `${overdueLoans.length} overdue` : 'On track'} />
        <KpiCard title="Customers" value={customers.length}
          description={`${overdueLoans.length} overdue loan${overdueLoans.length !== 1 ? 's' : ''}`}
          icon={Users} trend={overdueLoans.length > 0 ? 'down' : 'up'}
          trendValue={`${activeLoans.length} active`} />
      </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">#{getLoanNumber(loan)}</span>
                    </div>
